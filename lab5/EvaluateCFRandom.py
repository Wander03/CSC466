...
Description:
    How to run: python3 EvaluateCFRandom.py
...
def eval_random(D, method, size, repeats, KNN):
    ...
    for j in range(repeats):
        ...
        while num < size:
            ...
            u = users.sample(1).values[0]
            i = items.sample(1).values[0]
...
def main():
    ...
    if len(args) < 3:
        print("Syntax: python3 EvaluateCFRandom.py <jester-data-1.csv> <Method> <Size> <Repeats> <OPTIONAL: KNN>")
    ...
    if method == 5 or method == 6:
        try:
            ...
            print('Please Specify KNN Parameter')
            return None
    else:
        KNN = 0
